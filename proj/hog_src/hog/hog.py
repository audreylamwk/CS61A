...
def swine_align(player_score, opponent_score):
    ...
    gcd = 1
    d = 1
    while (d <= min(player_score, opponent_score)):
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while (score0 < goal and score1 < goal):
        if (who == 0):
            r = strategy0(score0, score1)
            score0 += take_turn(r, score1, dice)
            extr = extra_turn(score0, score1)
        else:
            r = strategy1(score1, score0)
            score1 += take_turn(r, score0, dice)
            extr = extra_turn(score1, score0)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def func(score0, score1):      
        ...
        rise = score - last_score
        gain = running_high
        if (rise > running_high):
            gain = rise
...
def bacon_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    pig = free_bacon(opponent_score)
    return 0 if pig >= cutoff else num_rolls # Replace this statement
    # END PROBLEM 10
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    nwscr = free_bacon(opponent_score)
    extr = extra_turn(score + nwscr, opponent_score)
    if (extr):
        return 0
